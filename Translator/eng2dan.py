...
encoder_model = load_model("Models/encodere2d.h5")
decoder_model = load_model("Models/decodere2d.h5")
encoder_model.compile(optimizer="adam", loss="categorical_crossentropy")
decoder_model.compile(optimizer="adam", loss="categorical_crossentropy")
# Define eng_vocab and dan_vocab dictionaries here
...
def clean(text):
    ...
    return text
@app.post("/translate")
async def translate(english: str = Form(...)):
    english = english.lower()
    english = clean(english)
    english = list(english.split(" "))
    
    if len(english) > 15:
        warning = "Please input up to 15 words"
        answer = ""
    else:
        r = encoding(english)
        if isinstance(r, str):
            ...
            padded_r = padding(r, 15)  # Pad the input sequence
            answer = Decode_sequence(padded_r)
            warning = ""
